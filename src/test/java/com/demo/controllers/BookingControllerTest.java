...
class BookingControllerTest {
    ...
    void setUp(){
        ...
        b1= Booking.builder()
                ...
                .checkIn(LocalDateTime.of(2026,4,22,15,30))
                .checkOut(LocalDateTime.of(2026,4,26,15,30))
                .guest(user)
                .build();
        b2= Booking.builder().guest(user).listing(apartamento).status(BookingStatus.PENDING).checkIn(LocalDateTime.of(2026,3,22,15,30)).checkOut(LocalDateTime.of(2026,3,26,15,30)).build();
        b3 = Booking.builder().guest(user).listing(apartamento).status(BookingStatus.PENDING).checkIn(LocalDateTime.now()).checkOut(LocalDateTime.now()).build();
    ...
    void detectarBooking() throws Exception{
        ...
        mockMvc.perform(get("/bookings/" + b1.getId()).with(user(user)))
    ...
    void listaBooking() throws Exception{
        ...
        mockMvc.perform(get("/bookings").with(user(user)))
                .andExpect(status().isOk())
    ...
    void confirmarBooking()throws Exception{
        ...
        mockMvc.perform(post("/bookings/"+b2.getId()+"/confirm").with(csrf()).with(user(user)))
                .andExpect(status().is3xxRedirection())
    ...
    void cancelarBooking()throws Exception{
        ...
        mockMvc.perform(post("/bookings/"+b3.getId()+"/cancel").with(csrf()).with(user(user)))
                .andExpect(status().is3xxRedirection())
